...
enum Apl_Return_Types apl_setup(struct Apl_Window_State *ws)
{
    ...
    strncpy(file_path, "../src/assets/obj/axis.obj", ASM_MAX_LEN_LINE);
    ada_appand(Tri_mesh, scene->original_tri_meshes, ae_tri_mesh_get_from_file(file_path));
    
    strncpy(file_path, "../src/assets/stl/teapot.stl", ASM_MAX_LEN_LINE);
    ada_appand(Tri_mesh, scene->original_tri_meshes, ae_tri_mesh_get_from_file(file_path));

    strncpy(file_path, "../src/assets/stl/Stanford dragon lowres.STL", ASM_MAX_LEN_LINE);
    ada_appand(Tri_mesh, scene->original_tri_meshes, ae_tri_mesh_get_from_file(file_path));

    strncpy(file_path, "../src/assets/obj/f16/f16.obj", ASM_MAX_LEN_LINE);
    ada_appand(Tri_mesh, scene->original_tri_meshes, ae_tri_mesh_get_from_file(file_path));
    ...
    ae_tri_mesh_rotate_Euler_xyz(scene->in_world_tri_meshes.elements[1], -90, 0, 180);
    ae_tri_mesh_rotate_Euler_xyz(scene->in_world_tri_meshes.elements[2], -90, 0, 180);
    ae_tri_mesh_rotate_Euler_xyz(scene->in_world_tri_meshes.elements[3], 0, 0, 180);

    ae_tri_mesh_translate(scene->in_world_tri_meshes.elements[0], 2, 0, 4);
    ae_tri_mesh_translate(scene->in_world_tri_meshes.elements[1], -2, 0, 4);
    ae_tri_mesh_translate(scene->in_world_tri_meshes.elements[2], 0, 0, 6);
    ae_tri_mesh_translate(scene->in_world_tri_meshes.elements[3], 0, 0, 2);
...
enum Apl_Return_Types apl_render(struct Apl_Window_State *ws)
{
    ...
    if (ws->mouse.right_button_is_pressed) {
        ae_z_buffer_copy_to_screen(ws->window_pixels_mat, ws->inv_z_buffer_mat);
    }
    for (size_t i = 0; i < scene->projected_tri_meshes.length; i++) {
        if (ws->mouse.left_button_is_pressed) {
            adl_tri_mesh_draw(ws->window_pixels_mat, scene->projected_tri_meshes.elements[i], 0xffff00ff, ADL_DEFAULT_OFFSET_ZOOM);
        }
    }
