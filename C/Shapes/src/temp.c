...
void setup(game_state_t *game_state)
{
    ...
    temp_implicit_mesh = as_points_array_get_lexicographic_triangulation(c.elements, c.length);

    // Curve temp_circle = as_circle_curve_create((Point){0,0,0,0}, 1, 10, 0xff000000, "xy");
    // temp_implicit_mesh = as_points_array_get_lexicographic_triangulation(temp_circle.elements, temp_circle.length);
    ...
    for (size_t i = 0; i < mesh.length; i++) {
        ...
        Curve temp_curve = as_circle_curve_create(center, r, 10, RGBA_hexARGB(255.0f * t, 255 * (1 - t), 255, 255), "xy");
        Curve temp_proj_curve = as_circle_curve_create(center, r, 10, RGBA_hexARGB(255.0f * t, 255 * (1 - t), 255, 255), "xy");
