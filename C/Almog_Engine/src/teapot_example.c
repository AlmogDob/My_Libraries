...
void setup(game_state_t *game_state)
{
    ...
    // ae_translate_mesh(game_state->scene.in_world_tri_meshes.elements[0], 0, 0, 2);
}
...
void render(game_state_t *game_state)
{
    for (size_t i = 0; i < game_state->scene.projected_tri_meshes.length; i++) {
        adl_fill_tri_mesh_Pinedas_rasterizer(game_state->window_pixels_mat, game_state->inv_z_buffer_mat, game_state->scene.projected_tri_meshes.elements[i], 0xffffffff, ADL_DEFAULT_OFFSET_ZOOM);
    }
